...
void Huffman::buildFreqMap(string filename)
{
	...
	if (file) {												       // si il n'y a pas d'erreur de flags
		char c;
		while (file.good())									       // Tant qu'on est pas � la fin du fichier
		{
			file.get(c);									       // on lit caractere par caractere
			pair<char, int> word(c, 1);							   // nouvelle paire cl�/valeur (freq initialisee a 1)
			map<char, int>::iterator search = charFreq->find(c);   // on place un it�rateur au caractere (place a la fin s'il n'est pas trouve)
...
/**
	Retourne le nombre d'octets th�orique
	apres compression de Huffman
 */
int Huffman::getTheoryNbBytes() const
{
	int nbBytes = 0;											// valeur de retour
	string tmp = "";											// caractere courrant
	map<char, int>::iterator itFreq = charFreq->begin();
	
	for (auto &itCode : huffmanCode)
	{
		itFreq  = charFreq->find(itCode.first);		// on cherche le caractere
		tmp     = itCode.second;					// code associe au caractere courrant
		nbBytes += itFreq->second * tmp.length();	// calcul du nombre de bits
	}

	nbBytes /= 8;		// conversion enti�re en octets
	return nbBytes;
}
 ...
 */
void Huffman::writeEncodedData(string filename, vector<unsigned char> *hexCode)
{
